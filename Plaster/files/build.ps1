param
(
    # To specify which tasks to run.
    [Parameter()]
    [string[]] $TaskList = 'default',
    # Skip dependency (modules) checks.
    [Parameter()]
...
Invoke-Psake -BuildFile "$PSScriptRoot\psake.ps1" -Parameters @{'ModuleName' = $ModuleName } -nologo -TaskList $TaskList
